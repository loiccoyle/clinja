...
""")
    ...
    def test_add(self):
        ...
        res = runner.invoke(cli.add, ['var1', 'value1_changed'], obj=self.obj,
                            input='n\n')
        self.assertEqual(res.exit_code, 0)
        self.assertTrue('var1' in self.static.stored.keys())
        self.assertEqual(self.static.stored['var1'], 'value1')
    ...
    def test_test(self):
        ...
        res = runner.invoke(cli.test,
                            ['--template', '../test',
                             '--destination', '../../../test',
                             '--run_cwd', 'somewhere/else',
                             '--static_vars', '{"c": 3}'],
                            obj=self.obj)
        output = res.output.strip().split('\n')
        static_content = self.static_content.replace('\n', ' ').replace('"', '\'')
    ...
    def test_run(self):
        runner = CliRunner()
        res = runner.invoke(cli.run, [str(self.template_path), '--prompt', 'never'],
                            obj=self.obj)
        self.assertEqual(res.exit_code, 1)
        self.assertTrue('missing' in res.output)

        res = runner.invoke(cli.run, [str(self.template_path), '--prompt', 'missing'],
                                    obj=self.obj, input='value_missing\ny\n')
        self.assertEqual(res.exit_code, 0)
        self.assertEqual(res.output,
                        f'''\
...
value_missing''')
        ...
        res = runner.invoke(cli.add, ['missing', 'value_missing'],
                            obj=self.obj)
        self.assertEqual(res.exit_code, 0)
...
value_missing''')
        ...
        res = runner.invoke(cli.run, [str(self.template_path), '--prompt', 'always'],
                            obj=self.obj, input='\n\n\n\n')
        self.assertEqual(res.exit_code, 0)
        self.assertEqual(res.output,
                f'''\
